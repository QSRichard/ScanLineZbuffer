...
ScanLine_zbuffer* Display::engine = NULL;
Model* Display::model = NULL;
Point3d Display::point_1st = Point3d(0.0, 0.0, 0.0);
Point3d Display::point_2rd = Point3d(0.0, 0.0, 0.0);
float** Display::RotateMat = NULL;

int Display::idx1 = -1, Display::idx2 = -1;
string path = "C:\\Users\\86173\\Desktop\\CG\\hw\\ScanLine_zbuffer\\z_buffer\\Project2\\models";
...
Display::Display(Model* model, ScanLine_zbuffer* slzBuffer)
{
	this->model = model;
	engine = slzBuffer;
...
void Display::loop()
{
	...
	for (int y = 0; y < height; ++y)
	{
		for (int x = 0; x < width; ++x)
		{
			...
			int tmp = engine->Color_id[y][x];
			if (tmp >= 0)
			{
				rgb = model->Model_faces[tmp].clolor;
...
//		Point3d center_point(width / 2, height / 2, 0.0);
