...
{
	Point3d min_xyz(0xfffffff, 0xfffffff, 0xfffffff),
		max_xyz(-0xfffffff, -0xfffffff, -0xfffffff);//7个f
	Point3d center_xyz(0.0, 0.0, 0.0);
	int vertex_num = model.Model_vertexs.size();
	for (int i = 0; i < vertex_num; ++i)
	{
		const Point3d& vertex = model.Model_vertexs[i].point;
...
#pragma omp parallel for //num_threads(NUM_THREADS)
	for (int i = 0; i < vertex_num; ++i)
	{
		Point3d& vertex_point = model.Model_vertexs[i].point;
		//放置到窗口中间
	...
	model.center_point = Point3d(width / 2, height / 2, 0);
}
